<!-- Navigation Bar -->
<nav class="navbar navbar-expand-lg navbar-dark sticky-top">
  <div class="container">
    <a class="navbar-brand cvsm" href="#">
      <img src="assets/images/cvsm-logo.png" alt="College Logo" style="width:40px;" class="rounded-pill"> 
      CVSM
    </a>

    <!-- Hamburger Bar -->
    <button class="navbar-toggler custom-toggler" type="button" data-bs-toggle="offcanvas" data-bs-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="custom-toggler-icon"><i class="bi bi-list fs-2"></i></span>
    </button>

    <!-- Offcanvas Sidebar -->
    <div class="offcanvas offcanvas-end" tabindex="-1" id="navbarNav" aria-labelledby="navbarNavLabel">
      <div class="offcanvas-header">
        <h5 class="offcanvas-title" id="navbarNavLabel">Menu</h5>
        <button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas" aria-label="Close"></button>
      </div>
      <div class="offcanvas-body">
        <ul class="navbar-nav ms-auto">

          <li class="nav-item">
            <a class="nav-link active" href="#">Home</a>
          </li>
          

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">About</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Vision & Mission</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">History</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Objectives</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Organization</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Administration</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Departments</a></li>
            </ul>
          </li>

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Academic Programs</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Doctor of Veterinary Medicine</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Master of Veterinary Medicine</a></li>
            </ul>
          </li>

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Research</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">On-going Project</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Publication</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Research Partners</a></li>
            </ul>
          </li>

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Updates</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">News</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Events & Announcements</a></li>
            </ul>
          </li>

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Resource & Facilities</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Lecture Halls</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Laboratories</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">CenTrAD</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">e-Histology</a></li>
            </ul>
          </li>

          <li class="nav-item dropdown">
            <a class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown">Extension</a>
            <ul class="dropdown-menu">
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Vet Mission</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Cattle Dispersal</a></li>
              <li><a class="dropdown-item" href="assets/coming-soon/index.html">Veterinary Teaching Hospital</a></li>
            </ul>
          </li>

        </ul>
      </div>
    </div>
  </div>
</nav>
